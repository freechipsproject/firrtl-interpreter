...
import firrtl_interpreter.Driver
import firrtl_interpreter.utils.VcdRunner
import firrtl_interpreter.vcd.VCD
import firrtl_interpreter.{FirrtlRepl, FirrtlTerp}

import scala.tools.jline.console.ConsoleReader
import scala.util.matching.Regex

class ReplVcdController(val repl: FirrtlRepl, val engine: FirrtlTerp, val vcd: VCD) {
  ...
  val vcdRunner: VcdRunner = new VcdRunner(repl.currentInterpreterOpt.get, vcd)
  ...
  def run(parameters: Array[String]): Unit = {
    parameters.toList match {
      ...
      case "to" :: tail => tail match {
        ...
        case "step" :: _ =>
          while(vcdRunner.hasNextEvent && !doChanges()) {
