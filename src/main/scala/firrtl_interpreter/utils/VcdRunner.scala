...
import firrtl_interpreter.{FirrtlTerp, InterpretiveTester}
import firrtl_interpreter.vcd.VCD
import firrtl_interpreter.vcd.Wire

/**
  * Executes a vcd file as a script against a [[InterpretiveTester]] instance.
 *
  * @param tester  the circuit to be updated
  * @param vcd     the vcd values to use
  */
class VcdRunner(val engine: FirrtlTerp, val vcd: VCD) {
  val events : Array[Long]     = vcd.events
  val inputs : Set[String]     = engine.dependencyGraph.inputPorts.toSet
  val outputs: Set[String]     = engine.dependencyGraph.outputPorts.toSet
  ...
  def hasName(symbolName: String): Boolean = {
    engine.dependencyGraph.nameToExpression.contains(symbolName)
  }

  def setValue(wire: Wire, newValue: BigInt): Unit = {
    val fullName = wire.fullName
    if(hasName(fullName)) {
      engine.setValueWithBigInt(fullName, newValue)
      println(s"$fullName <= ${engine.getValue(fullName)}")
      inputValuesSet += 1
    }
  }
  ...
  def checkValue(wire: Wire, newValue: BigInt): Unit = {
    ...
    if(hasName(fullName)) {
      ...
      val result = if(engine.getValue(fullName).value == newValue) {
        testSuccesses += 1
        "ok"
      }
      else {
  ...
  def setAllWires(eventNumber: Int): Unit = {
    ...
    engine.circuitState.isStale = false
  }
